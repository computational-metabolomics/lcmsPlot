...
Until now, we’ve been using an `XCMSnExp` object as our data source. However, `lcmsPlot` also supports extracting chromatograms directly from raw files, such as those in the `mzML` format.
...
To select multiple scans across a detecte peak, given an RT interval, you can use `mode = "across_peak"`:
