...
```{r}
ms1_samples_metadata <- samples_metadata %>% filter(ms_level == 1)
...
```{r}
ms2_samples_metadata <- samples_metadata %>% filter(ms_level == 2)
...
my_custom_theme <- function() {
  theme(
    ...
    # Axis labels
    axis.title = element_text(size = 11, face = "bold", color = "#1d293d"),
    ...
    strip.text = element_text(
      ...
      size = 9,
      hjust = 0,
      margin = margin(t = 3, r = 10, b = 6, l = 10)
    ),
    strip.background = element_rect(color = "#90a1b9", fill = "#e2e8f0", size = 0.8),

    # Box around the panel
    panel.border = element_rect(color = "#90a1b9", fill = NA, linewidth = 1),

    # Customize grid lines inside the plot
    panel.grid.major = element_line(color = "#e2e8f0", linewidth = 0.5),
    panel.grid.minor = element_line(color = "#e2e8f0", linewidth = 0.5)
  )
}
```
