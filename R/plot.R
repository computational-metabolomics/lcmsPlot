...
run_matching_plot_variant <- function(datasets, obj) {
    if (length(datasets) == 0) {
        warning("No data to plot: all requested datasets are empty.")
        return(
    ...
    # When isolation_window is active, use the annotated spectrum renderer
    spectra_fn <- if (isTRUE(obj@options$isolation_window$show)) {
    ...
    plot_config <- list(
        ...
        purity_timeline = plot_purity_timeline,
        purity_distribution = plot_purity_distribution
    )
